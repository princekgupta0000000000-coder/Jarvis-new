...
/** Offline Gemma/GGUF bridge backed by the llama.cpp Android binding. */
object JarvisAiEngine {
    private var engine: InferenceEngine? = null
    private var loadedPath: String? = null

    suspend fun ask(context: Context, prompt: String, onToken: (String) -> Unit): Result<Unit> = runCatching {
        val inference = ensureLoaded(context)
        inference.sendUserPrompt(prompt, predictLength = 512).collect { token -> onToken(token) }
    }
    ...
    suspend fun ensureLoaded(context: Context): InferenceEngine = withContext(Dispatchers.IO) {
        ...
        val name = (prefs.getString("name", null) ?: "jarvis-model.gguf").replace(Regex("[^A-Za-z0-9._-]"), "_")
        ...
        if (!target.exists() || (sourceLength > 0 && target.length() != sourceLength)) {
            context.contentResolver.openInputStream(uri)?.use { input -> target.outputStream().use { output -> input.copyTo(output, DEFAULT_BUFFER) } }
                ?: error("JARVIS cannot read the selected GGUF file.")
        }

        val current = engine
        if (current != null && loadedPath == target.absolutePath && current.state.value is InferenceEngine.State.ModelReady) return@withContext current
    ...
    private fun contentLength(context: Context, uri: Uri): Long = runCatching {
        context.contentResolver.query(uri, arrayOf(android.provider.OpenableColumns.SIZE), null, null, null)?.use { c -> if (c.moveToFirst()) c.getLong(0) else -1L } ?: -1L
    }.getOrDefault(-1L)
